...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      // Получаем информацию о объявлении, преобразуем listingId в целое число
      ...
      if (listing.publisherId === userId) { // Убедитесь, что это правильное поле
      ...
      const existingResponse = await prisma.response.findFirst({
        where: {
          responderId: userId,
          listingId: parseInt(listingId), // Преобразуем также здесь
        },
      });
      ...
      const responses = await prisma.response.findMany({
        where: { listingId: parseInt(listingId) }, // Используем parseInt для приведения типа
      });
